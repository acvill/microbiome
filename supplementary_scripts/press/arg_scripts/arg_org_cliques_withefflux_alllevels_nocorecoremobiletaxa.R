...
#i don't know how to handle this well...for this one i want it to be the combos
sub5$patients <- as.vector(sub5$Sample)
...
for (i in 0:max(sub7_multipat$Patcount)-1){
	print(i)
...
colors <- c("white","purple","blue","lightblue","yellow","orange","red","pink")
heat.col = colors[0:max(subdata_multipat)+i1]
...
for (i in seq(length(pats))){
	plot(patient_graphs[[i]],layout=l,vertex.label=NA,vertex.frame.color=V(patient_graphs[[i]])$enterocol,vertex.size=vsize,vertex.color=V(patient_graphs[[i]])$phylumcolor,edge.color=kewl[i],edge.width=E(patient_graphs[[i]])$weight_2/line_scaling)
par(new=TRUE)
m=max(m,E(patient_graphs[[i]])$weight_2)
}
legend("bottomright",cex = 4,pch=21,legend=c(pp$Phylum),col=c(pp$Phylumcolor),pt.bg=c(pp$Phylumcolor))
legend("topleft",cex = 5,pch=1,legend=c("Enterobacteriaceae","Not Enterobacteriaceae"),col=c("orange","blue"))
legend("topright",cex=5,lty=1,lwd=20,legend=pats,col=c(kewl))
