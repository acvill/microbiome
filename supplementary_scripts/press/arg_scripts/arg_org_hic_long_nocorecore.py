...
outhandle = '{0}/arg_v_org/metagenomes/histograms/nocorecore/arg_org_hic_separated_{1}_{2}_{3}_{4}.tbl'.format('/workdir/users/agk85/' + folder , argpid , hicpid ,str(depth), str(2))
...
for combo in combos:
	...
	for i in range(len(delims)):
		arghisthandle = '/workdir/users/agk85/' + folder + '/arg_v_org/metagenomes/histograms/nocorecore/arg_orgcounts_'+ hic + '+' + mgm + '_' + argpid + '_' + hicpid + '_' + delim_names[i] + str(depth) + '.txt'
		delim = delims[i]
		arghistcount = {}
		level_orgs = []
		for org in orglist:
...
for i in range(len(delims)):
	arghisthandle = '/workdir/users/agk85/' + folder + '/arg_v_org/metagenomes/histograms/nocorecore/arg_orgcounts_'+ argpid + '_' + hicpid + '_' +  delim_names[i] + str(depth) + '.txt'
	delim = delims[i]
	arghistcount = {}
	level_orgs = []
	for org in orglist:
