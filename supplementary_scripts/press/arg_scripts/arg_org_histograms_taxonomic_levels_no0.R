...
setwd(paste("/workdir/users/agk85/", folder, "/arg_v_org/metagenomes/histograms/",folder2,sep=""))
combos = read.csv(paste("/workdir/users/agk85/",folder,"/ComboDesign.txt",sep=""),header=F,sep="\t")
combos$V3 = c(rep("_", length(combos$V1)))
clmns <- paste(combos$V1, combos$V2, sep="+")
clmns <- paste(clmns, combos$V3, sep="")
patients = c(clmns, "")
...
delims = c("species")
figs <- vector("list", length(patients) * 3) 
keepcount <- vector("list", 7)
stats <- vector("list", length(patients))
...
delimname = delims[i]
print(delimname)
indata0 = read.csv(paste("arg_orgcounts_", patient, argpid, "_", hicpid, "_", delimname, "0.txt",sep=""), sep="\t",  header=F)
indata1 = read.csv(paste("arg_orgcounts_", patient, argpid, "_", hicpid, "_", delimname, "1.txt",sep=""), sep="\t", row.names=1, header=F)
indata2 = read.csv(paste("arg_orgcounts_", patient, argpid, "_", hicpid, "_", delimname, "2.txt",sep=""), sep="\t", row.names=1, header=F)
...
figs[[j]]= a
figs[[j+1*length(patients)]]= b
figs[[j+2*length(patients)]]= c
}

pdf(paste("All_ARG_ORG_histogram_no0_",folder, "_",folder2,"_",delimname,".pdf",sep=""), height = 20, width =15)
grid.arrange(figs[[1]], figs[[4]], figs[[7]],
        figs[[2]], figs[[5]],figs[[8]],
        figs[[3]], figs[[6]],figs[[9]],
	ncol=3, nrow=length(patients), widths=c(2.5, 5, 8), heights=c(rep(2,length(patients))))
dev.off()
#stats matrix
...
colnames(m)<-header
rownames(m)<-patients
write.table(m, paste("ARG_histogram_",delimname, "_stats.txt",sep=""),quote = FALSE,sep ="\t")
}
