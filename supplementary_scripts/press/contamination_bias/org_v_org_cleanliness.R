...
methods = c(pid,paste(pid, "_mge",sep=""))

theme_set(theme_nogrid())
workdir=paste("/workdir/users/agk85/",folder,sep="")
setwd(paste(workdir,"/hic/plotting/",sep=""))

for (method in methods){
df <- read.csv(paste(folder, "_org_v_org_", method, ".txt", sep=""),header=T,row.names=1,sep="\t")
...
pdf(paste(folder, "_heatmap_org_v_org_", method, ".pdf",sep=""), height=50, width=50,useDingbats=FALSE)
...
write.table(unique(s), paste(folder, "_taxapresent_", taxa, ".txt",sep=""))
df2 <- t(aggdata)
...
pdf(paste(folder,"_heatmap_org_v_org_lev_", taxa, "_", method, ".pdf",sep=""), height=20, width=20,useDingbats=FALSE)
heatmap.2((mat2), hclustfun=myclust, distfun=mydist, na.rm = TRUE, scale="none",
...
pdf(paste(folder, "_heatmap_org_v_org_lev_", taxa, "_", method, "+higherlevels.pdf",sep=""), height=20, width=20,useDingbats=FALSE)
heatmap.2((mat2), hclustfun=myclust, distfun=mydist, na.rm = TRUE, scale="none",
        dendrogram="none", Rowv=F, Colv=F, key=TRUE, symbreaks=FALSE, margins=c(30,30),
        symkey=F, density.info="none", trace="none", labCol=colnames(mat2), labRow=rownames(mat2),
        cexRow=.8, cexCol=.8, col=my_palette, RowSideColors=rsc, ColSideColors=rsc)
