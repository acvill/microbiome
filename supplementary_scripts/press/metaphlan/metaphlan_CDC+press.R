...
#########set variable things
folder='/workdir/users/agk85/press/metaphlan'
infile='/workdir/users/agk85/press/metaphlan/CDC+Press_metaphlan.txt'
pairfile = '/workdir/users/agk85/press/metaphlan/mgm_hic_pairs_cdc+press.csv'
generalname = 'CDC+Press'
...
keepers <- c("B370.1","B370.1hic","B370.2","B370.2hic","B370.3","B370.3hic",
        ...
        "B320.3","B320.3hic","B320.4","B320.4hic","B320.5","B320.5hic",
...
all_mgm_hic_pre$prepost <- rep("pre", nrow(all_mgm_hic_pre))
all_mgm_hic_post <- subset(all_mgm_hic, patient %in% c("Sau3aI","MluCI","ProxiMeta"))
all_mgm_hic_post$prepost <- rep("post", nrow(all_mgm_hic_post))
all_mgm_hic_both<- rbind(all_mgm_hic_pre,all_mgm_hic_post)
...
g<-ggplot(data=all_mgm_hic_both, aes(r,diff))+
	...
	ylim(-100,100)+
	scale_shape_manual("Restriction Enzyme Round",labels=c("pre"="CDC", "post"="ProxiMeta"), values=c(19,17))+
	scale_color_manual("Patient", values=c("blue","mediumblue","midnightblue","skyblue3","steelblue3","turquoise2","navy","violetred2","red"))+
...
pdf(paste(generalname, "_stacked_",taxalev,".pdf",sep=""), height = 5*l, width=6*l,useDingbats=FALSE)
