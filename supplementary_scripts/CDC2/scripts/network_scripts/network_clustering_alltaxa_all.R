...
infile<-paste(workdir, "/outputs/Stats_withmge_", name2, "_", identity, "_", cut.off,".txt", sep="") 
...
write.csv(g7df, paste(workdir, '/memberships/',name2, "_min", cut.off,"_",identity,"_addon_species_membership_withmge.txt",sep=""))
####################################################################
...
pdf(paste(name2, "_min", cut.off,"_",identity,"_addon_membership_withmge.pdf", sep=""), height = 50, width = 50)
...
g7_filename <- paste(workdir, "/graphs/Graph_withmge_", name2, "_", identity, "_", cut.off,".txt", sep="")
...
write(paste(c(name, identity, cutoff, conflictleveltracker,ecount(g_no_mges)),collapse='\t'), file=paste('/workdir/users/agk85/', folder, '/networks/outputs/Conflict_level_tracking_withmges.txt',sep=""), append=T)
