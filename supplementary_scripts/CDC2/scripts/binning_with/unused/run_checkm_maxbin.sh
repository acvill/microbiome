#$ -S /bin/bash
#$ -N checkm_with
#$ -e /workdir/users/agk85/CDC2/logs/maxbin_checkm_with_$JOB_ID.err
#$ -o /workdir/users/agk85/CDC2/logs/maxbin_checkm_with_$JOB_ID.out
#$ -t 1-43
#$ -V
...
FOLDER=$WRK/maxbin_with/${NAME}
OUT=$WRK/maxbin_with/${NAME}/checkm_lineage

 if [ ! -d $OUT ]; then mkdir -p $OUT; fi

LOG=$WRK/logs/maxbin_checkm_with_$JOB_ID_${NAME}.out

echo start checkm ${NAME} `date` >> $LOG 2>&1

#run checkm
source  /programs/checkm_161011/init
checkm lineage_wf -t 8 -x fasta /workdir/users/agk85/CDC2/maxbin_with/${NAME} $OUT
