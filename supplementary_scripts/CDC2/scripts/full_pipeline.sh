...
####PATRIC_COMPARISONS####
#pay attention becasue this one is super dumb about taxids 
#ok we blasted on cbsubrito 
#~/agk/CDC_sidejobs/blast_100919.sh #or something like that
#output is in BLAST_OUT folder
#but i ran on the server as the first time it didn't finish---no errors so be careful
# scp args_99_nr_1e-100.txt agk85@cbsubrito2.tc.cornell.edu:~/agk/CDC2/args/patric_comparisons
#then we add in the taxids
#i have no idea how i made the convert_patric_taxid script so...trust

#this take into account that the same gene+taxonomy combos might come from multiple das file interactions
#make the hic support connections figures with association_type included
...
#now conver the blast ids to taxids
...
######i got tired so there are some missing figures
...
cp ~/agk/CDC2/das/*.pdf ~/CDC_figures/binning_figures
cp ~/agk/CDC2/das/*bins_stats.txt ~/CDC_figures/binning_figures
...
cp ~/agk/CDC2/bins/diversity_figures/*.pdf ~/CDC_figures/diversity_figures
...
cp ~/agk/CDC2/args/patric_comparisons/*.pdf ~/CDC_figures/toparg_figures
...
cp ~/agk/CDC2/read_distributions/*.pdf ~/CDC_figures/read_distributions
cp ~/agk/CDC2/read_distributions/*statistics.txt ~/CDC_figures/read_distributions
