...
outhandle = '{0}/arg_v_org/metagenomes/histograms/nocorecorenomobilemobile/arg_org_hic_separated_{1}_{2}_{3}_{4}.tbl'.format('/workdir/users/agk85/' + folder , argpid , hicpid ,str(depth), str(2))
...
for pat in patients:
	for i in range(len(delims)):
		delim = delims[i]
		arghistcount = {}
		arghisthandle = '/workdir/users/agk85/' + folder + '/arg_v_org/metagenomes/histograms/nocorecorenomobilemobile/arg_orgcounts_lumped_'+ pat + '_' + argpid + '_' + hicpid + '_' + delim_names[i] + str(depth) + '.txt'
		for combo in combos:
			hic = combo[0]
			mgm = combo[1]
			patient = hic.split('-')[0]
...
for i in range(len(delims)):
	arghisthandle = '/workdir/users/agk85/' + folder + '/arg_v_org/metagenomes/histograms/nocorecorenomobilemobile/arg_orgcounts_lumped_'+ argpid + '_' + hicpid + '_' + delim_names[i] + str(depth) + '.txt'
	delim = delims[i]
	arghistcount = {}
	level_orgs = []
	for org in orglist:
