...
outhandle = '{0}/arg_v_org/metagenomes/cliques/nocorecorenomobilemobile/dnaG_org_hic_cliques_{1}_{2}_{3}_{4}.tbl'.format('/workdir/users/agk85/' + folder ,argpid , hicpid ,str(args.depth), str(2))
...
header = 'Count\tCluster\tARG_name\tSample\tOrganism'
outhandle = '{0}/arg_v_org/metagenomes/cliques/nocorecorenomobilemobile/dnaG_org_hic_list_{1}_{2}_{3}_{4}.tbl'.format('/workdir/users/agk85/' + folder ,argpid , hicpid ,str(args.depth), str(2))
with open(outhandle, 'w') as outfile:
	outfile.write(header)
	for key in clusterprotdict.keys():
		argname='dnaG'
		for combo in combos:
			...
			count += 1
			for organism in orglist:
				if organism in argdict[cb][key]:
					outfile.write('{0}\t{1}\t{2}\t{3}\t{4}\n'.format(str(count), key, argname,hic, organism))
