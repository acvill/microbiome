...
for combo in combos:
	...
	for i in range(len(delims)):
		arghisthandle = '/workdir/users/agk85/' + folder + '/arg_v_org/metagenomes/histograms/nocorecorenomobilemobile/arg_orgcounts_'+ hic + '+' + mgm + '_' + argpid + '_' + hicpid + '_' + delim_names[i] + str(depth) + '.txt'
		delim = delims[i]
		arghistcount = {}
		level_orgs = []
		for org in orglist:
		...
		with open(arghisthandle, 'w') as argout:
			for arg in clusterprotdict.keys():
				if hic in clusterpatientdict[arg]:
...
for i in range(len(delims)):
	arghisthandle = '/workdir/users/agk85/' + folder + '/arg_v_org/metagenomes/histograms/nocorecorenomobilemobile/arg_orgcounts_'+ argpid + '_' + hicpid + '_' + delim_names[i] + str(depth) + '.txt'
	delim = delims[i]
	arghistcount = {}
	level_orgs = []
	for org in orglist:
