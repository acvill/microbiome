...
def getOptions():
 	"""Get arguments"""
 	description="""This script can be used to get arg versus organism capturing orgs on the contig and contigs up to N links away via Hi-C reads, this one lumps patients together instead of separating them apart"""
...
samples = []
samplist = glob.glob('/workdir/users/agk85/' + folder + '/todoCDC/*')
for samplong in samplist:
	samp = samplong.split('/')[6]
	samples.append(samp)

inhandle = '/workdir/users/agk85/' + folder + '/arg_v_org/metagenomes/dnaG_' + argpid + '_nr.fna.clstr'
protclusterdict = {}
clusterprotdict = {}
clusterpatientdict = {}
clusternum_map = {}
...
outhandle = '{0}/arg_v_org/metagenomes/histograms/nocorecorenomobilemobile/dnaG_org_hic_separated_{1}_{2}_{3}_{4}.tbl'.format('/workdir/users/agk85/' + folder , argpid , hicpid ,str(depth), str(2))
...
#really you want the pat instead of hic+mgm hic+mgm instead of pat
for pat in patients:
	for i in range(len(delims)):
		delim = delims[i]
		arghistcount = {}
		arghisthandle = '/workdir/users/agk85/' + folder + '/arg_v_org/metagenomes/histograms/nocorecorenomobilemobile/dnaG_orgcounts_lumped_'+ pat + '_' + argpid + '_' + hicpid + '_' + delim_names[i] + str(depth) + '.txt'
		for combo in combos:
			hic = combo[0]
			mgm = combo[1]
			patient = hic.split('-')[0]
...
for i in range(len(delims)):
	arghisthandle = '/workdir/users/agk85/' + folder + '/arg_v_org/metagenomes/histograms/nocorecorenomobilemobile/dnaG_orgcounts_lumped_'+ argpid + '_' + hicpid + '_' + delim_names[i] + str(depth) + '.txt'
	delim = delims[i]
	arghistcount = {}
	level_orgs = []
	for org in orglist:
