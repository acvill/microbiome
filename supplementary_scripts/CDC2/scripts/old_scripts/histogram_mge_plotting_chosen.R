...
inhandle = 'Together_das_1_mgetaxa_together.txt'
...
s = subset(subfile, TaxaCount>1 & Level == "s__")
delimname = 's__'
lev = 'species'
df = s
...
ylab("MGE contig Taxa Counts")+
scale_x_continuous(breaks=seq(2,19,1))+
facet_grid(Patient~Binner+Thresh+Bintype+Residency,scales = "free_y")

pdf(paste("MGE_ORG_histogram_species_min1_min", min_contact, "_",sample,".pdf",sep=""), height = 15, width =10)
...
agg$Total_mges <- rep(df$Total_mges[1],nrow(agg))
print(agg)
write.table(agg, paste("MGE_Numbers_","min1_min",min_contact, "_",sample,".txt",sep=""), row.names=F,quote=FALSE,sep="\t")
...
print(agg)
write.table(agg, paste("MGE_Taxa_Count_Numbers_","min1_min",min_contact, "_",sample,".txt",sep=""),row.names=F, quote=FALSE,sep="\t")
...
write.table(agg, paste("MGE_Metrics_","min1_min",min_contact, "_",sample,".txt",sep=""), row.names=F,quote=FALSE,sep="\t")
