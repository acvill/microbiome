...
infile = paste(genetype, "_", minreads,"_patient_pairwise_connection_counts.txt",sep="")
...
df5.dist <- vegdist(as.matrix(df4), method="bray", binary=T)
...
pdf(paste("Patient_pairwise_diversity_vs_connectivity_",genetype,"_",minreads,".pdf",sep=""),height=5,width=8,useDingbats=FALSE)
ggplot(div_con,aes(x=Species_Dissimilarity_Bray,y=Connectivity))+
ylab("Pairwise Patient Gene-Species Connectivity")+
