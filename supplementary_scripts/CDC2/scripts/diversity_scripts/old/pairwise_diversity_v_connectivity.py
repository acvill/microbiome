		...
		for sample2 in samples:
			for level in levels:
				if (sample2,sample1,level) not in finished:
					patient1 = sample1.split('-')[0]
					patient2 = sample2.split('-')[0]
					if (patient1 in healthy and patient2 in healthy):
						comparison = 'healthy-healthy'
					...
					if (patient1 in sick and patient2 in sick):
						comparison = 'sick-sick'
					if (patient1 == patient2 and patient1 in healthy):
						comparison = 'self-healthy'
					if (patient1 == patient2 and patient1 in sick):
						comparison = 'self-sick'
					...
					outfile.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\n'.format(sample1, sample2, level,comparison,str(number_of_shared_connections),str(number_of_shared_taxa),str(number_of_shared_genes)))
