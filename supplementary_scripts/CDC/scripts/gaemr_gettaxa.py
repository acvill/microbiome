...
scaffolds= '/workdir/users/agk85/CDC/prodigal_excise/metagenomes3/' + name + '/' + name + '_scaffold.fasta'
scfdict = SeqIO.to_dict(SeqIO.parse(scfs, "fasta"))
scaffolddict = SeqIO.to_dict(SeqIO.parse(scaffolds, "fasta"))
scflink = {}
count = 0
...
for contig in contig_taxa.keys():
	...
	gaemrscf = contig_scaffold[contig]
	...
	try:
		try:
			scftaxalen[gaemrscf][taxa] = scftaxalen[gaemrscf][taxa] + taxalength
		except KeyError:
			scftaxalen[gaemrscf][taxa] = taxalength
	except:
		scftaxalen[gaemrscf] = {}
		scftaxalen[gaemrscf][taxa] = taxalength
...
for gaemrscf in scftaxalen.keys():
	...
	for taxa in scftaxalen[gaemrscf].keys():
		newl = scftaxalen[gaemrscf][taxa]
		if newl >= l:
			longesttaxa = taxa
			l = newl	
	scftaxa[gaemrscf] = longesttaxa



outhandle = infolder + name + '/gaemr/table/' + name + '.scf.taxa.txt'
with open(outhandle,'w') as outfile:
	for gaemrscf in scftaxa.keys():
		realscf = scflink[gaemrscf]
		taxa = scftaxa[gaemrscf]
		replace1 = taxa.replace(',','')
		replace2 = replace1.replace("'", "")
		replace3 = replace2.replace("#","")
		outfile.write(realscf + '\t' + replace3 + '\n')
