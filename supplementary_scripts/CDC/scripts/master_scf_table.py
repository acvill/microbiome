...
protdict = {}
with open('/workdir/users/agk85/CDC/prodigal/metagenomes/' + name + '/' + name + '_proteins.faa','r') as prot:
	for line in prot:
		if line[0] == '>':
...
ids = []
with open('/workdir/users/agk85/CDC/prodigal/metagenomes/' + name + '/' + name + '_scaffold.fasta','r') as infile:
	for line in infile:
		if line[0] == '>':
...
# final aggregation
linedict = {}
header = '1.ScfID\t2.Cbar(type)\t3.Plasmid_Finder(pid|start_stop)\t4.Full_Plasmids(pid|start_stop)\t5.Relaxase(eval|start_stop_rpkm_covg)\t6.Resfams(type|eval|start_stop_rpkm_covg)\t7.Perfect(pid|start_stop)\t8.CARD(arg|start_stop_rpkm_covg)\t9.ISEScan(type|start_stop_rpkm_covg)\t10.Phage_Finder(start_stop)\t11.Phaster(pid|start_stop)\t12.Vir_Finder(pval)\t13.VOGS(vog|bit|start_stop_rpkm_covg)\t14.Amphora(best_taxonomy(confidence))\n'
for id in ids:
	linedict[id] = id  #1
...
#write linedict
outfile = open('/workdir/users/agk85/CDC/combo_tables/metagenomes/' + name + '_master_scf_table.txt','w')
