...
with open(outhandle,'w') as outfile:
	...
	for j in range(len(switches)):
		...
		delims = [';.', '; s__','; g__','; f__','; o__','; c__','; p__']
		...
		for i in range(len(delims)):
			...
			for inline in infile:
				...
				if fields[1] + fields[2] + fields[3] + fields[4] + fields[8] + fields[9] + fields[10] + fields[14] + fields[15] + fields[16] + fields[19] + fields[20] == '............':
					if inline[0] !='S':
						besttaxa = '.'
						maxscore = 0
						r = 0
						...
						if 'rnammer_' in inline:
							...
							if (switch[0] == '1' and 's__;' not in org):
								contig_taxonomies.append(org.split(delim)[0])
							if ((pid > maxscore) and ('s__;' not in org)):
								besttaxa = org
								maxscore = score
			...
			try:
				outfile.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}\t{11}\t{12}\n'.format(switch_names[j], delim_names[i], noannot, oneplusannot, mannot, oannot, gannot, maddedannot, any_taxonomies, multiple_taxonomies, conflicting_taxonomies, str(round(100*float(conflicting_taxonomies)/any_taxonomies,2)),str(round(100*float(conflicting_taxonomies)/multiple_taxonomies,2))))
			except ZeroDivisionError:
				outfile.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}\t{11}\t{12}\n'.format(switch_names[j], delim_names[i], noannot, oneplusannot, mannot, oannot, gannot, maddedannot, any_taxonomies, multiple_taxonomies, conflicting_taxonomies,'divisionerror?' ,'divisionerror?'))
