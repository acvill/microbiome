...
type = sys.argv[1]
type = 'genome'
paths = glob.glob('/workdir/users/agk85/CDC/masurca/*/CA/10-gapclose/genome.scf.fasta')
...
outfile = open('/workdir/users/agk85/CDC/masurca/log/' + type + '_statistics.txt','w')
...
for path in paths:
        ...
        outfile.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n'.format(root,reads,str(scf_count),str(total),str(mean),str(median),str(maxvalue),str(n50)))
