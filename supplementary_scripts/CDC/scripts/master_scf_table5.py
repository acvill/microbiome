...
def best_org(inline):
	...
	if 'mtphln_' in inline:
		mtphln_hits = inline.split('\t')[19].split('mtphln_')[1].split(',')
...
for scfid in ids:
	try:
		mtphln = mtphln_dict[scfid]
		mtphln_hits = mtphln.split(',')
		taxa = []
		pids = []
		for item in mtphln_hits:
			if 's__;' not in item:
				taxon = item.split('|')[1]
				pid = item.split('|')[0]
		 		taxa.append(taxon)
		...
		#of the pids associated with this taxa, get the highest one
		linked = []
		for mosttaxon in mosttaxa:
			...
			try:
				mtphln_single_dict[scfid] = mtphln_single_dict[scfid] + ',' + str(maxpid) + '|' + mosttaxon
			except KeyError:
				mtphln_single_dict[scfid] = str(maxpid) + '|' + mosttaxon
	except KeyError:
		continue
...
for id in ids:
	...
	add_program(linedict, mtphln_single_dict,id, "mtphln_") #20
	add_program(linedict, maxbin_dict, id, "maxbin_") #21
#write linedict
