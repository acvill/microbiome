...
with open(inhandle) as infile:
	for line in infile:
		...
		lowest, lowest_level = lowest_taxonomy(taxonomy)
		taxalevel_dict[(lowest, lowest_level)] = taxonomy
		if '_sp._' in primary:
			specieslevel_dict[(primary, lowest_level-1)] = taxonomy
...
with open('/workdir/users/agk85/CDC/rnammer/metagenomes3/' + name + '_16S.out.best.withtaxa.txt','r') as rnammer:
	...
	for line in rnammer:
		...
		try:
			...
			ncbitaxa = taxalevel_dict[(lowest, lowest_level)]
			newtaxa = ncbitaxa
		except KeyError:
			newtaxa = newtaxa
