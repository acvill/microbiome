...
with open('/workdir/users/agk85/CDC/prodigal_excise/metagenomes2/combined_prots/' + patient + '_nr.fna.clstr','r') as infile:
	clusterdict = {}
	clusternum_map = {}
	for line in infile:
		if line[0] == '>':
...
with open('/workdir/users/agk85/CDC/rnammer/metagenomes2/' + name + '_16S.out.best.withtaxa.txt','r') as rnammer:
	...
	for line in rnammer:
		...
		taxa = line.split('\t')[12].split('__')[-2].split()[-1] + '__' + line.split('\t')[12].split('__')[-1].split(';')[0]
		try:
			rnammer_dict[id] = rnammer_dict[id] +',' + taxa + '_' +pid + '_' + b + '_' + e
		except KeyError:
			rnammer_dict[id] = taxa + '_' +pid + '_' + b + '_' + e
...
# final aggregation
linedict = {}
header = '1.ScfID\t2.Plasmid_Finder(pid|start_stop)\t3.Full_Plasmids(pid|start_stop)\t4.Relaxase(eval|start_stop_rpkm_covg)\t5.Resfams(type|eval|start_stop_rpkm_covg)\t6.Perfect(pid|start_stop)\t7.CARD(arg|start_stop_rpkm_covg)\t8.ISEScan(type|start_stop_rpkm_covg)\t9.Phage_Finder(start_stop)\t10.Phaster(pid|start_stop)\t11.VOGS(vog|bit|start_stop_rpkm_covg)\t12.VF(gene|pid|start_stop_rpkm_covg)\t13.tRNASCAN(trna_cove_start_stop)\t14.Imme(element_pid_start_stop)\t15.Aclame_plasmid(element_pid_start_stop)\t16.Aclame_phage(element_pid_start_stop)\t17.Amphora(best_taxonomy(confidence))\t18.Rnammer(taxa_pid_start_stop)\n'
for id in ids:
	linedict[id] = id  #1
