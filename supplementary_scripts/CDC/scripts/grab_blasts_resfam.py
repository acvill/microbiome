...
#patient = 'B316'
#clusterfile = '/workdir/users/agk85/CDC/resfams/metagenomes/patients/B316_resfam_args_nr.fna.clstr'
clusters = {}
...
card_info = {}
paths = glob.glob('/home/agk85/agk/CDC/resfams/metagenomes/*/*.dom.txt')
for hmm_result  in paths:
	hmm_resfams = SearchIO.parse(hmm_result, 'hmmscan3-domtab')
	for rec in hmm_resfams:
		protid = rec.id
		for hit in rec:
			info = hit.description
		try:
			card_info[protid].append(info)
		except KeyError:
			card_info[protid] = [info]

for scf,info in card_info.iteritems():
	card_info[scf] = '|'.join(info)
...
for cluster, prots in clusters.iteritems():
	...
	if len(cluster_blasts[cluster]) != 0 and overlap>0:
		...
		scfs.sort()
		recs = []
		handle = 'clusters/' + patient + '_cluster_' + str(cluster) + '.fna'
		for scf in scfs:
			try:
