#GOAL grabl all of the proteins that are mobile element related, 
#USAGE: python protseqs_ofinterest.py B314-1
####################################################################################
...
def update_with_scaffold(start, stop, scf, mgetype):
	with open('/workdir/users/agk85/CDC/prodigal_excise/metagenomes3/' + name + '/' + name + '_proteins.fna') as pf:
		flag = 0
		for protline in pf:
			if protline[0] == '>':
				...
				if scf.split('_')[2] == protline.split('_')[2]:
					...
					#if they overlap and if the overlapping section covers more than 50% of gene length
					if (start < e and stop > b and (float(min(e-start, stop -b, stop-start, e-b)))/(e-b)>0.5):
						...
						flag = 1
		if flag == 0:
			print(scf, start, stop)

print(name)
protfile = '/workdir/users/agk85/CDC/prodigal_excise/metagenomes3/' + name + '/' + name + '_proteins.fna'
for rec in SeqIO.parse(protfile, "fasta"):
	protdict[rec.id] = str(rec.seq)

#protdict_old = {}
#protfile = '/workdir/users/agk85/CDC/prodigal/metagenomes3/' + name + '/' + name + '_proteins.fna'
#for rec in SeqIO.parse(protfile, "fasta"):
...
with open('/workdir/users/agk85/CDC/plasmids/metagenomes3/' + name + '/' + name + '_plasmidgenes_filter.out') as plasmid_finder: 
...
print('fullplasmids')
with open('/workdir/users/agk85/CDC/plasmids/metagenomes3/' + name + '/' + name + '_fullplasmids_filter.out','r') as fullplasmids:
...
with open('/workdir/users/agk85/CDC/imme/metagenomes3/'  + name + '/' + name + '_imme_filter.out','r') as imme:
	for line in imme:
		...
		if 'Prophage' in immedb[hit]:
			mgetype = 'phage'
			update_with_scaffold(start, stop, scf, mgetype)
		if 'IME' in immedb[hit] or 'ICE' in immedb[hit]:
			mgetype = 'plasmid'
			update_with_scaffold(start, stop, scf, mgetype)
		else:
			continue
...
print('plasmid_pfams')
with open('/workdir/users/agk85/CDC/plasmids/metagenomes3/' + name + '/' + name + '_plasmid_pfam.txt','r') as plasmidpfams:
...
ht = 0
lht = 0
with open('/workdir/users/agk85/CDC/tables/metagenomes3/' + name + '_mge.fna', 'w') as mge_outfile:
	for key,entry in mgedict.iteritems():
		...
		if 'transposon' in types:
			transposon = '1'
		mge_outfile.write('>' + key + ' ' + plasmid + '|' + phage + '|' + transposon +'\n' + seq + '\n')
...
print("lht:",float(lht)/count, lht, count)
print("lh:",float(lh)/count, lh, count)
print("lt:",float(lt)/count, lt, count)
print("ht:",float(ht)/count, ht, count)
