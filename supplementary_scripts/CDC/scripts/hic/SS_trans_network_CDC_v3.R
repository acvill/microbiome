...
library(qgraph)
library(ggplot2)
setwd('/workdir/users/agk85/CDC/sureselect/plotting/')
...
names = read.table(paste(workdir,'/SureSelectDesign.txt',sep=""), header=F)
names = as.character(names$V1)
for (name in names){
print(name)
#trans interactions from hic data
data <- read.table(paste(workdir, "/sureselect/mapping/", name,"/",name, "_trans_primary_", identity, ".txt",sep=""), sep="\t")
colnames(data)<-c("ID1","Flag1","Contig1","Start1","Mapq1","CIGAR1","ID2","Flag2","Contig2","Start2","Mapq2","CIGAR2")

annotations <- read.table(paste(workdir, "/combo_tables/metagenomes3/", name,"_master_scf_table_binary.txt",sep=""),sep="\t",header=T,row.names=1)

amph <- read.table(paste(workdir, "/combo_tables/metagenomes3/", name,"_master_scf_table.txt",sep=""),quote = "", sep="\t",header=T,row.names=1)

plasmids <- annotations$Plasmid_Finder + annotations$Full_Plasmids + annotations$Relaxase + annotations$Aclame_plasmid + annotations$Imme_plasmid + annotations$Plasmid_pfams
args <- annotations$Resfams + annotations$Perfect + annotations$CARD
is <- annotations$ISEScan + annotations$Imme_transposon
phage <- annotations$Phage_Finder  + annotations$Phaster + annotations$Imme_phage + annotations$Aclame_phage
...
l <- layout_nicely(graph.to.plot)


########change this back when you want to replot data 
pdf(paste("SS_trans_nicely_annot_withlink_", name,"_", identity, "_",cut.off, ".pdf",sep=""),height=100, width=100)
