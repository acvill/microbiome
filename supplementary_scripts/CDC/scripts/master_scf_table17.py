...
#other_pfams
with open('/workdir/users/agk85/CDC/annotation/metagenomes3/other_pfams/' + name + '_other_pfam.txt','r') as other_pfams:
	other_pfam_dict = {}
...
with open('/workdir/users/agk85/CDC/mtphln/metagenomes3/' + name + '/'+name+'_metaphlan.out.best.filter', 'r') as mtphln:
	with open('/workdir/users/agk85/CDC/mtphln/metagenomes3/'+name + '/' + name + '_metaphlan.no_ncbi_taxid.txt','w') as bad_mtphln:
		mtphln_dict = {}
		for line in mtphln:
			idinfo = line.split('\t')[0].split('_')
...
with open('/workdir/users/agk85/CDC/annotation/metagenomes3/'+name+'_mobilegenes.txt', 'r') as mobile_pfam:
	mobile_pfam_dict = {}
...
for id in ids:
	...
	add_program(linedict, mobile_pfam_dict,id, "mobilepfam_") #29
	add_program(linedict, mobile_pfam_plasmid_dict, id, "7plasmid_")#30
	add_program(linedict, mobile_pfam_phage_dict, id, "5phage_")#31
	add_program(linedict, mobile_pfam_transposon_dict, id, "2transposon_")#32
	add_program(linedict, mobile_pfam_other_dict, id, "1other_")#33
	add_program(linedict, gaemr_dict, id, "gaemr_")#34
...
for id in ids:
	...
	if ((fields[1] + fields[2] + fields[3] + fields[4] + fields[8] + fields[9] + fields[10] + fields[14] + fields[15] + fields[16] + fields[19] + fields[20]+fields[28]) == '.............') and ('Viruses' not in bestorg):
		linedict[id] = linedict[id] + '\t.\t.'
	elif ('k__Viruses' in bestorg):
		linedict[id] = linedict[id] + '\tVirus_taxonomy\tmge' #34
	else: 
		linedict[id] = linedict[id] + '\t.\tmge' #35
	linedict[id] = linedict[id] + '\t' + bestorg #36


outfile = open('/workdir/users/agk85/CDC/combo_tables/metagenomes5/' + name + '_master_scf_table.txt','w')
eukfile = open('/workdir/users/agk85/CDC/combo_tables/metagenomes5/' + name + '_euk.txt','w')
