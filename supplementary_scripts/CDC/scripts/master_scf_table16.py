...
#phaster
with open('/workdir/users/agk85/CDC/phage/metagenomes3/' + name + '/' + name + '_phaster_5060_filter.out','r') as phaster:
	phaster_dict = {}
	for line in phaster:
		idinfo = line.split('\t')[0].split('_')
		id = idinfo[0] + '_' + idinfo[1] + '_' + idinfo[2]
...
#aclame
with open('/workdir/users/agk85/CDC/aclame/metagenomes3/' + name + '/' + name + '_aclame_5060_filter.out','r') as aclame:
	aclame_plasmid_dict = {}
	aclame_phage_dict = {}
	for line in aclame:
		idinfo = line.split('\t')[0].split('_')
...
for id in ids:
	...
	if ((fields[1] + fields[2] + fields[3] + fields[4] + fields[8] + fields[9] + fields[10] + fields[14] + fields[15] + fields[16] + fields[19] + fields[20]+fields[28]) == '.............') and ('Viruses' not in bestorg):
		...
		linedict[id] = linedict[id] + '\tVirus_taxonomy\tmge' #30 #31
	else: 
		linedict[id] = linedict[id] + '\t.\tmge' #30 #31
	linedict[id] = linedict[id] + '\t' + bestorg #32


outfile = open('/workdir/users/agk85/CDC/combo_tables/metagenomes4/' + name + '_master_scf_table.txt','w')
eukfile = open('/workdir/users/agk85/CDC/combo_tables/metagenomes4/' + name + '_euk.txt','w')
